...
#just do this if movement couldn't be satisfied due to wall or something.
	if not move_cooldown and not forward_raycast.get_collider() == null:
		on_move_fail.emit()
	#last check checks so that there isnt a wall or object blocking movement.
	elif not move_cooldown and forward_raycast.get_collider() == null:
		if not moving:
...
#endregion
#region Rotation
	if not forward_raycast.get_collider() == null and not move_cooldown:
		if not rotating:
			last_rotation = quaternion
			var random_direction = randi_range(0, 1) * 2 - 1
			direction_of_rotation = (Vector3.UP * random_direction).normalized()
